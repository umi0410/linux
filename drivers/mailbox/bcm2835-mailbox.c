...
static irqreturn_t bcm2835_mbox_irq(int irq, void *dev_id)
{
    ...
    // NOTE: 기존에는 무조건 IRQ_HANDLED를 리턴했기에 IRQ Thread가 호출되지도 않았음. (물론 IRQ Thread가 존재하지도 않았음.)
	//return IRQ_HANDLED;
    // NOTE: 이제는 IRQ_WAKE_THREAD를 리턴함으로써 무조건 IRQ Thread가 후반부로 실행되도록 함.
...
static int bcm2835_mbox_probe(struct platform_device *pdev)
{
    ...
    // NOTE: 기존에는 devm_request_irq로 IRQ Thread 없이 interrupt를 처리했음.
	//ret = devm_request_irq(dev, platform_get_irq(pdev, 0),
	//		       bcm2835_mbox_irq, 0, dev_name(dev), mbox);
    // NOTE: 이제는 bcm2835_mbox_threaded_irq를 IRQ Thread로 설정함.
